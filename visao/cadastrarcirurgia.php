...
<html lang="en">
    ...
    <center>
        <div class="container">
            <div class=row>
                <div class="col-sm-21">
                    <h2>Cadastro de Cirurgia</h2>
                </div>
            </div>
    </center>
    <div class="row">
        <div class="col-sm-offset-3">
            <form class="form-horizontal" action="cadastrarcirurgia.php" method="post" role="form">
                ...
                <div class="form-group">
                    ...
                    <div class="col-sm-5">          
                        <input type="text" name="dataCirurgia" class="form-control" id="datacirurgia">
                        <span class="help-block"></span>
                    </div>
                </div>
                <div class="form-group">
                ...
                <div class="form-group">
                    ...
                    <div class="col-sm-5">          
                        <input type="text" name="tempo" id="num" class="form-control">
                        <span class="help-block"></span>
                    </div>
                </div>
                <div class="form-group">
                ...
                <div class="form-group">
                    ...
                    <div class="col-sm-offset-3">          
                        <input type="submit" name="cadastrar" class="btn btn-default btn-lg" value="Cadastrar">
                    </div>
                </div>
            </form>
        </div>
    ...
    if (isset($_POST["same"]) && isset($_POST["tempo"]) && isset($_POST["situacao"]) && isset($_POST["Obito"]) && isset($_POST["proteses"]) && isset($_POST["dataCirurgia"]) && isset($_POST["procedimento"]) && isset($_POST["unidade"]) && isset($_POST["same"])) {
        ...
        if ($paciente->Verifica_Paciente($_POST["same"]) > 0) {
            if ($paciente->Validardata($_POST["dataCirurgia"]) && is_numeric($_POST["tempo"])) {
                $cirurgia->setDatacirurgia($paciente->formatar_data($_POST["dataCirurgia"]));
                $cirurgia->setProteses($_POST["proteses"]);
                $cirurgia->setTempo($_POST["tempo"]);
                $cirurgia->setSituacao($_POST["situacao"]);
                $cirurgia->setObito($_POST["Obito"]);
                $cirurgia->setPaciente($_POST["same"]);
                $cirurgia->setProcedimento($_POST["procedimento"]);
                $cirurgia->setUnidade($_POST["unidade"]);
                $cirurgia->setTempo($_POST["tempo"]);
                $cirurgia->setSituacao($_POST["situacao"]);
                if ($cirurgiaDAO->inserir_Cirurgia($cirurgia)) {
  ...
  <span class=\"sr-only\">Error:</span>cadastrado com sucesso </div></div>";
                }
            } else {
  ...
  <span class=\"sr-only\">Error:</span>Data Invalida e/ou o valor do tempo esta irregular </div></div>";
            ...
            echo"<div class=\"row\"><div class=\"alert alert-danger\" role=\"alert\">
  <span class=\"glyphicon glyphicon-exclamation-sign\" aria-hidden=\"true\"></span>
  <span class=\"sr-only\">Error:</span>Paciente não esta cadastrado</div></div>";
        }
    }
    ?>
        </div>
